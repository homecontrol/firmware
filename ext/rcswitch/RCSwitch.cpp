...
bool RCSwitch::sendRaw(const char* code)
{
    noInterrupts();
    cli();
    ...
    int t = 0;
    for(int i = 0; i < this->nRepeatTransmit; i ++)
    {
        t = 0;
        int j = 0;
        const char* c = code;
        while(true)
        {
            if(*c == '.' || *c == 0)
            {
                if((j % 2) == 0) digitalWrite(this->nTransmitterPin, HIGH);
                else digitalWrite(this->nTransmitterPin, LOW);
                delayMicroseconds(t);
